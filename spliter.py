...
def startProc(statusDisplay):
    ...
    if 'filename' in procInfo and 'outputdir' in procInfo:
        ...
        statusDisplay.insert(str(dispIndex)+'.0', 'Wordpress WXR Spliter' + fileName + '\n');
        dispIndex += 1;
    ...
    while(EOF==False):
        ...
        if (currentCount+maxInc) < totalCount:
            ...
            statusDisplay.insert(str(dispIndex)+'.0', 'Finished... \n');
            dispIndex += 1;
            EOF = True;
...
if __name__ == '__main__':
    ...
    root = Tk(className='Wordpress WXR Spliter');
    root.geometry("400x150");
    root['background']='#fff';

    srcFileText = Label(root, text="Source: ").grid(row=0, column=0, sticky=W);
    outDirText  = Label(root, text="Destination: ").grid(row=1, column=0, sticky=W);
    ...
    browseSrcFile = Button(root, text="Select File", bg='#ffb3fe', command=getAndDispFileName).grid(row=0, column=12, sticky=W+E);
    browseOutDir  = Button(root, text="Select Dir.", command=getAndDispOutDir).grid(row=1, column=12, sticky=W+E);
       
    statusDisplay = Text(root, width = 50, height = 10);
    statusDisplay.grid(row=3, column=0, columnspan=13);
